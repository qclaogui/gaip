...
//
//	HTTP binding pattern: POST "/v1/users"
func (srv *Server) HandleCreateUser() http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		urlPathParams := mux.Vars(r)
		numURLPathParams := len(urlPathParams)

		_ = level.Debug(srv.logger).Log("msg", fmt.Sprintf("Received %s request matching '/v1/users': %q", r.Method, r.URL))
		...
		ctx := context.WithValue(r.Context(), resttools.BindingURIKey, "/v1/users")
...
//
//	Generated for HTTP binding pattern: GET "/v1/{name=users/*}"
func (srv *Server) HandleGetUser() http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		urlPathParams := mux.Vars(r)
		numURLPathParams := len(urlPathParams)

		_ = level.Debug(srv.logger).Log("msg", fmt.Sprintf("Received %s request matching '/v1/{name=users/*}': %q", r.Method, r.URL))
		_ = level.Debug(srv.logger).Log("msg", fmt.Sprintf("urlPathParams (expect 0, have %d): %q", numURLPathParams, urlPathParams))
		...
		ctx := context.WithValue(r.Context(), resttools.BindingURIKey, "/v1/{name=users/*}")
...
//
//	Generated for HTTP binding pattern: PATCH "/v1/{user.name=users/*}"
func (srv *Server) HandleUpdateUser() http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		urlPathParams := mux.Vars(r)
		numURLPathParams := len(urlPathParams)

		_ = level.Debug(srv.logger).Log("msg", fmt.Sprintf("Received %s request matching '/v1/{user.name=users/*}': %q", r.Method, r.URL))
		_ = level.Debug(srv.logger).Log("msg", fmt.Sprintf("urlPathParams (expect 0, have %d): %q", numURLPathParams, urlPathParams))
		if numURLPathParams != 1 {
			rest.Error(w, http.StatusBadRequest, "found unexpected number of URL variables: expected 0, have %d: %#v", numURLPathParams, urlPathParams)
			return
		...
		ctx := context.WithValue(r.Context(), resttools.BindingURIKey, "/v1/{user.name=users/*}")
...
//
//	Generated for HTTP binding pattern: DELETE "/v1/{name=users/*}"
...
//
//	Generated for HTTP binding pattern: GET "/v1/users"
