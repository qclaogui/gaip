...
//
//	HTTP binding pattern: GET "/v1/operations"
func (srv *Server) HandleListOperations() http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		urlPathParams := mux.Vars(r)
		numURLPathParams := len(urlPathParams)

		_ = level.Info(srv.logger).Log("msg", fmt.Sprintf("Received %s request matching '/v1/operations': %q", r.Method, r.URL))
		...
		ctx := context.WithValue(r.Context(), resttools.BindingURIKey, "/v1/operations")
...
//
//	HTTP binding pattern: GET "/v1/{name=operations/**}"
func (srv *Server) HandleGetOperation() http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		urlPathParams := mux.Vars(r)
		numURLPathParams := len(urlPathParams)

		_ = level.Info(srv.logger).Log("msg", fmt.Sprintf("Received %s request matching '/v1/{name=operations/**}': %q", r.Method, r.URL))
		_ = level.Info(srv.logger).Log("msg", fmt.Sprintf("urlPathParams (expect 1, have %d): %q", numURLPathParams, urlPathParams))
		...
		ctx := context.WithValue(r.Context(), resttools.BindingURIKey, "/v1/{name=operations/**}")
...
//
//	HTTP binding pattern: DELETE "/v1/{name=operations/**}"
...
//
//	HTTP binding pattern: POST "/v1/{name=operations/**}:cancel"
func (srv *Server) HandleCancelOperation() http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		urlPathParams := mux.Vars(r)
		numURLPathParams := len(urlPathParams)

		_ = level.Info(srv.logger).Log("msg", fmt.Sprintf("Received %s request matching '/v1/{name=operations/**}:cancel': %q", r.Method, r.URL))
		_ = level.Info(srv.logger).Log("msg", fmt.Sprintf("urlPathParams (expect 1, have %d): %q", numURLPathParams, urlPathParams))
		...
		ctx := context.WithValue(r.Context(), resttools.BindingURIKey, "/v1/{name=operations/**}:cancel")
