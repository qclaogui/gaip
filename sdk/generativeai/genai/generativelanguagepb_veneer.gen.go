...
// BlockReason is specifies the reason why the prompt was blocked.
...
const (
	...
	// BlockReasonSafety means prompt was blocked due to safety reasons. Inspect `safety_ratings`
	...
	// BlockReasonBlocklist means prompt was blocked due to the terms which are included from the
	// terminology blocklist.
	BlockReasonBlocklist BlockReason = 3
	// BlockReasonProhibitedContent means prompt was blocked due to prohibited content.
	BlockReasonProhibitedContent BlockReason = 4
	// BlockReasonImageSafety means candidates blocked due to unsafe image generation content.
	BlockReasonImageSafety BlockReason = 5
)
...
var namesForBlockReason = map[BlockReason]string{
	...
	BlockReasonBlocklist:         "BlockReasonBlocklist",
	BlockReasonProhibitedContent: "BlockReasonProhibitedContent",
	BlockReasonImageSafety:       "BlockReasonImageSafety",
}
...
func (v *Content) toProto() *pb.Content {
	...
	return &pb.Content{
		Parts: pvTransformSlice(v.Parts, (*Part).toProto),
		Role:  v.Role,
	}
}
...
func (Content) fromProto(p *pb.Content) *Content {
	...
	return &Content{
		Parts: pvTransformSlice(p.Parts, (Part{}).fromProto),
		Role:  p.Role,
	}
}
...
func (v *File) toProto() *pb.File {
	...
	p := &pb.File{
		...
		Uri:            v.URI,
		DownloadUri:    v.DownloadUri,
		State:          pb.File_State(v.State),
		Source:         pb.File_Source(v.Source),
...
const (
	// FileSourceUnspecified means used if source is not specified.
	FileSourceUnspecified FileSource = 0
	// FileSourceUploaded means indicates the file is uploaded by the user.
	FileSourceUploaded FileSource = 1
	// FileSourceGenerated means indicates the file is generated by Google.
	FileSourceGenerated FileSource = 2
)

var namesForFileSource = map[FileSource]string{
	FileSourceUnspecified: "FileSourceUnspecified",
	FileSourceUploaded:    "FileSourceUploaded",
	FileSourceGenerated:   "FileSourceGenerated",
}

func (v FileSource) String() string {
	if n, ok := namesForFileSource[v]; ok {
		return n
	}
...
const (
	...
	// FinishReasonSafety means the response candidate content was flagged for safety reasons.
	FinishReasonSafety FinishReason = 3
	// FinishReasonRecitation means the response candidate content was flagged for recitation reasons.
	FinishReasonRecitation FinishReason = 4
	// FinishReasonLanguage means the response candidate content was flagged for using an unsupported
	// language.
	FinishReasonLanguage FinishReason = 6
	// FinishReasonOther means unknown reason.
	FinishReasonOther FinishReason = 5
	// FinishReasonBlocklist means token generation stopped because the content contains forbidden terms.
	FinishReasonBlocklist FinishReason = 7
	// FinishReasonProhibitedContent means token generation stopped for potentially containing prohibited content.
	FinishReasonProhibitedContent FinishReason = 8
	// FinishReasonSpii means token generation stopped because the content potentially contains
	// Sensitive Personally Identifiable Information (SPII).
	FinishReasonSpii FinishReason = 9
	// FinishReasonMalformedFunctionCall means the function call generated by the model is invalid.
	FinishReasonMalformedFunctionCall FinishReason = 10
	// FinishReasonImageSafety means token generation stopped because generated images contain safety
	// violations.
	FinishReasonImageSafety FinishReason = 11
)
...
var namesForFinishReason = map[FinishReason]string{
	...
	FinishReasonLanguage:              "FinishReasonLanguage",
	...
	FinishReasonImageSafety:           "FinishReasonImageSafety",
}
...
const (
	...
	// FunctionCallingValidated means model decides to predict either a function call
	// or a natural language response, but will validate function calls with
	// constrained decoding.
	FunctionCallingValidated FunctionCallingMode = 4
)
...
var namesForFunctionCallingMode = map[FunctionCallingMode]string{
	...
	FunctionCallingValidated:   "FunctionCallingValidated",
}
...
func (v *GenerationConfig) toProto() *pb.GenerationConfig {
	...
	return &pb.GenerationConfig{
		...
		ResponseLogprobs:           v.ResponseLogprobs,
		Logprobs:                   v.Logprobs,
		EnableEnhancedCivicAnswers: v.EnableEnhancedCivicAnswers,
	}
}
...
func (GenerationConfig) fromProto(p *pb.GenerationConfig) *GenerationConfig {
	...
	return &GenerationConfig{
		...
		ResponseLogprobs:           p.ResponseLogprobs,
		Logprobs:                   p.Logprobs,
		EnableEnhancedCivicAnswers: p.EnableEnhancedCivicAnswers,
	}
}
...
const (
	...
	// HarmBlockSafetysettingOff means turn off the safety filter.
	HarmBlockSafetysettingOff HarmBlockThreshold = 5
)
...
var namesForHarmBlockThreshold = map[HarmBlockThreshold]string{
	...
	HarmBlockSafetysettingOff: "HarmBlockSafetysettingOff",
}
...
const (
	...
	// HarmCategoryDerogatory means **PaLM** - Negative or harmful comments targeting identity and/or protected
	// attribute.
	HarmCategoryDerogatory HarmCategory = 1
	// HarmCategoryToxicity means **PaLM** - Content that is rude, disrespectful, or profane.
	HarmCategoryToxicity HarmCategory = 2
	// HarmCategoryViolence means **PaLM** - Describes scenarios depicting violence against an individual or
	// group, or general descriptions of gore.
	HarmCategoryViolence HarmCategory = 3
	// HarmCategorySexual means **PaLM** - Contains references to sexual acts or other lewd content.
	HarmCategorySexual HarmCategory = 4
	// HarmCategoryMedical means **PaLM** - Promotes unchecked medical advice.
	HarmCategoryMedical HarmCategory = 5
	// HarmCategoryDangerous means **PaLM** - Dangerous content that promotes, facilitates, or encourages
	// harmful acts.
	HarmCategoryDangerous HarmCategory = 6
	// HarmCategoryHarassment means **Gemini** - Harassment content.
	HarmCategoryHarassment HarmCategory = 7
	// HarmCategoryHateSpeech means **Gemini** - Hate speech and content.
	HarmCategoryHateSpeech HarmCategory = 8
	// HarmCategorySexuallyExplicit means **Gemini** - Sexually explicit content.
	HarmCategorySexuallyExplicit HarmCategory = 9
	// HarmCategoryDangerousContent means **Gemini** - Dangerous content.
	HarmCategoryDangerousContent HarmCategory = 10
	// HarmCategoryCivicIntegrity means **Gemini** - Content that may be used to harm civic integrity.
	HarmCategoryCivicIntegrity HarmCategory = 11
)
...
// Modality is content Part modality
type Modality int32

const (
	// ModalityModalityModalityUnspecified means unspecified modality.
	ModalityModalityModalityUnspecified Modality = 0
	// ModalityModalityText means plain text.
	ModalityModalityText Modality = 1
	// ModalityModalityImage means image.
	ModalityModalityImage Modality = 2
	// ModalityModalityVideo means video.
	ModalityModalityVideo Modality = 3
	// ModalityModalityAudio means audio.
	ModalityModalityAudio Modality = 4
	// ModalityModalityDocument means document, e.g. PDF.
	ModalityModalityDocument Modality = 5
)

var namesForModality = map[Modality]string{
	ModalityModalityModalityUnspecified: "ModalityModalityModalityUnspecified",
	ModalityModalityText:                "ModalityModalityText",
	ModalityModalityImage:               "ModalityModalityImage",
	ModalityModalityVideo:               "ModalityModalityVideo",
	ModalityModalityAudio:               "ModalityModalityAudio",
	ModalityModalityDocument:            "ModalityModalityDocument",
}

func (v Modality) String() string {
	if n, ok := namesForModality[v]; ok {
		return n
	}
	return fmt.Sprintf("Modality(%d)", v)
}

// ModalityTokenCount is represents token counting info for a single modality.
...
func (v *ModalityTokenCount) toProto() *pb.ModalityTokenCount {
	if v == nil {
		return nil
	}
	return &pb.ModalityTokenCount{
		Modality:   pb.Modality(v.Modality),
		TokenCount: v.TokenCount,
	}
}

func (ModalityTokenCount) fromProto(p *pb.ModalityTokenCount) *ModalityTokenCount {
	if p == nil {
		return nil
	}
...
func (Part) fromProto(p *pb.Part) *Part {
	if p == nil {
		return nil
	}
	v := &Part{
...
func (v *Schema) toProto() *pb.Schema {
	...
	return &pb.Schema{
		...
		Minimum:          v.Minimum,
		Maximum:          v.Maximum,
		AnyOf:            pvTransformSlice(v.AnyOf, (*Schema).toProto),
		PropertyOrdering: v.PropertyOrdering,
		Default:          v.Default,
	}
}
...
func (Schema) fromProto(p *pb.Schema) *Schema {
	...
	return &Schema{
		...
		Minimum:          p.Minimum,
		Maximum:          p.Maximum,
		AnyOf:            pvTransformSlice(p.AnyOf, (Schema{}).fromProto),
		PropertyOrdering: p.PropertyOrdering,
		Default:          p.Default,
	}
}
...
const (
	...
	// TaskTypeCodeRetrievalQuery means specifies that the given text will be used for code retrieval.
	TaskTypeCodeRetrievalQuery TaskType = 8
)
...
var namesForTaskType = map[TaskType]string{
	...
	TaskTypeCodeRetrievalQuery: "TaskTypeCodeRetrievalQuery",
}
...
func (v *UsageMetadata) toProto() *pb.GenerateContentResponse_UsageMetadata {
	...
	return &pb.GenerateContentResponse_UsageMetadata{
		...
		ToolUsePromptTokenCount:    v.ToolUsePromptTokenCount,
		ThoughtsTokenCount:         v.ThoughtsTokenCount,
		TotalTokenCount:            v.TotalTokenCount,
		PromptTokensDetails:        pvTransformSlice(v.PromptTokensDetails, (*ModalityTokenCount).toProto),
		CacheTokensDetails:         pvTransformSlice(v.CacheTokensDetails, (*ModalityTokenCount).toProto),
		CandidatesTokensDetails:    pvTransformSlice(v.CandidatesTokensDetails, (*ModalityTokenCount).toProto),
		ToolUsePromptTokensDetails: pvTransformSlice(v.ToolUsePromptTokensDetails, (*ModalityTokenCount).toProto),
	}
}
...
func (UsageMetadata) fromProto(p *pb.GenerateContentResponse_UsageMetadata) *UsageMetadata {
	...
	return &UsageMetadata{
		...
		ToolUsePromptTokenCount:    p.ToolUsePromptTokenCount,
		ThoughtsTokenCount:         p.ThoughtsTokenCount,
		TotalTokenCount:            p.TotalTokenCount,
		PromptTokensDetails:        pvTransformSlice(p.PromptTokensDetails, (ModalityTokenCount{}).fromProto),
		CacheTokensDetails:         pvTransformSlice(p.CacheTokensDetails, (ModalityTokenCount{}).fromProto),
		CandidatesTokensDetails:    pvTransformSlice(p.CandidatesTokensDetails, (ModalityTokenCount{}).fromProto),
		ToolUsePromptTokensDetails: pvTransformSlice(p.ToolUsePromptTokensDetails, (ModalityTokenCount{}).fromProto),
	}
}
